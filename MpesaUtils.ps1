...
function New-MpesaConnection {
    Write-Host "🔌 Creating API connection..." -ForegroundColor Cyan
...
function Start-MpesaDemo {
    ...
    if (Test-MpesaConnection) {
        Write-Host "`n📈 Adding sample transactions..." -ForegroundColor Yellow
        ...
        foreach ($tx in $sampleTx) {
            try {
                ...
                Write-Host "  ✅ $($tx.Id): $$($tx.Amount)" -ForegroundColor Green
            } catch {
                Write-Host "  ❌ Failed: $($tx.Id)" -ForegroundColor Red
            }
        }
        
        # Show final analytics
        Write-Host "`n📊 Final Analytics:" -ForegroundColor Yellow
        Get-MpesaAnalytics
    }
...
function Quick-AddTransaction {
    ...
    Write-Host "⚡ Quick transaction: $Id" -ForegroundColor Cyan
...
# Initialize
Write-Host "✅ M-Pesa Analytics PowerShell Client" -ForegroundColor Green
Write-Host "📡 Server: $($MpesaConfig.BaseUrl)" -ForegroundColor Gray
Write-Host "👤 User: $($MpesaConfig.Username)" -ForegroundColor Gray
